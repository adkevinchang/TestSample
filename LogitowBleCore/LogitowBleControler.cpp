...
{
	printf("LogitowBleControler::ConnectDevice!/n");
	...
	if (charBufferSize > 0)
	{
		...
		if (hr == S_OK) {
			...
			printf("BluetoothGATTGetCharacteristics - Actual Data %d  %d \r\n", numChars, charBufferSize);
		}
	...
	for (int ii = 0; ii <charBufferSize; ii++) {
			...
			for (int kk = 0; kk<numDescriptors; kk++) {
				...
				if (currGattDescriptor->AttributeHandle < 255) {
					...
					if (HRESULT_FROM_WIN32(ERROR_NO_SYSTEM_RESOURCES) == hr) {
						...
						printf("BluetoothGATTSetDescriptorValue - ERROR_NO_SYSTEM_RESOURCESa %d\n", hr);
						//ConnectModelDevice();
					}
					if (HRESULT_FROM_WIN32(ERROR_INVALID_FUNCTION) == hr) {
						...
						printf("BluetoothGATTSetDescriptorValue - ERROR_INVALID_FUNCTION %d\n", hr);
						//ConnectModelDevice();
					}
					if (S_OK  == hr) {
						this->connected = true;
						OutputDebugPrintf("BluetoothGATTSetDescriptorValue - S_OK %d\n", hr);
						printf("BluetoothGATTSetDescriptorValue - S_OK %d\n", hr);
						//ConnectModelDevice();
					}
					if (S_OK != hr) {
						OutputDebugPrintf("BluetoothGATTSetDescriptorValue - S_OK != hr %d\n", hr);
						printf("BluetoothGATTSetDescriptorValue - S_OK != hr %d\n", hr);
						//ConnectModelDevice();
					}
		...
		if (currGattChar->IsNotifiable) {
			...
			printf("Setting Notification for ServiceHandle %d %s\n", currGattChar->ServiceHandle, BTH_LE_UUID_TO_STRING(currGattChar->CharacteristicUuid).c_str());
			...
			if (S_OK == hr) {
				...
				printf("BluetoothGATTRegisterEvent - ok %d\n", hr);
			}
			if (HRESULT_FROM_WIN32(ERROR_INVALID_PARAMETER) == hr) {
				OutputDebugPrintf("BluetoothGATTRegisterEvent - invalid parameter\n");
				printf("BluetoothGATTRegisterEvent - invalid parameter\n");
			}
		}
...
{
	printf("LogitowBleControler::ScanBLEDevices!/n");
	HDEVINFO hDI;
	SP_DEVINFO_DATA did;
	DWORD i;
